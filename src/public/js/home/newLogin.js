...
const USE = document.querySelectorAll(".buttonson")
const Kebord =  document.getElementById("kebord");
 const Close = document.getElementById("close")
 Kebord.addEventListener("click", KEbord);
 ...
 function KEbord() {
  ...
  const NumBER =  (kenuber = kenuber+1)%2;
  
console.log(NumBER, "djfjdf")
if(NumBER === 0) {
...
for(var u =0; u<box1Length; u++) {
    ...
    document.querySelector(".menuCategorys").getElementsByTagName("a")[u]
...
.then(data => {  
  ...
  document.querySelector('#bu').addEventListener("click" , bu)
console.log( document.querySelector('#bu'))
 const name = document.querySelector("#name")
 const phone =document.querySelector("#telephon")
 
console.log(data, "data71")
function bu() {
  const userIndex = data.name.indexOf(name.value)
  // const userPhone= data.filter(function (addSave) { return addSave.phon === phone.value});
  // const userName= data.filter(function (addSave) { return addSave.name === phone.name});
  if(userIndex >= 0) {
if(data.phon[userIndex] === phone.value){
  
    
    const comb = phone.value.substr(9, 10)
    const combination =  nowTimeSS + comb 
 console.log("kkk")


      alert("인증 요청하였습니다. 인증번호를 입력 하세요")
      const req = {
        Phone: data.phon[userIndex],
        id :data.id[userIndex],
        name :data.name[userIndex],
  ...
  fetch("/search", {
    ...
    .then((res => res.json()))
    .then((res) => {
         document.querySelector("#Number").addEventListener("click" , suuu) 
       const a =   document.querySelector("#certification")
function suuu(){
...
} else { alert( "전화번호가 다릅니다.")}

} else {alert("존재하는 이름이  없습니다.")}
          ...
          .then((data) => {
            ...
            if (data.success === false) {
             ...
             console.log("jjjiih")
              document.getElementById("goodsNmae").style = "display:block"
              document.querySelector(".expandables").style = "display:block"
              console.log( document.querySelector(".expandable-box"))
              ...
              if (data.a[0].goodsName.length > 0) {
                ...
                for (var i = 0; i < data.a[0].goodsName.length; i++) {
                  ...
                  if (data.a[0].goodsName[i] !== "") {
                    ...
                    document.getElementsByTagName("a")[8 + i].id = `aTag-${[i]}`
                    document.getElementById(`aTag-${[i]}`).setAttribute("cdId", [i])
                  
                  
                 
                    const index = document.getElementById(`aTag-${[i]}`).getAttribute("cdId")
                    document.getElementById(`aTag-${[i]}`).onclick = () =>('click' , myFunction(index) )
...
location = "/newLogin"
 ...
 USE.forEach((buttonson) => {

  buttonson.addEventListener('click', onClickBox)
  function onClickBox() {
    if(buttonson.value = "buy") {
